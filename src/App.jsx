...
function ChatBubble({msg,members,self,onReact,onReply,onDel,onStar,starred}){
  ...
  if(msg.del)return<div style={{display:"flex",justifyContent:isSelf?"flex-end":"flex-start",padding:"2px 16px",opacity:0.4}}><div style={{padding:"6px 14px",borderRadius:12,background:"#1e293b",fontStyle:"italic",fontSize:13,color:"#94a3b8"}}>🚫 Deleted</div></div>;
  ...
  return(
    <div style={{display:"flex",justifyContent:isSelf?"flex-end":"flex-start",padding:"3px 16px",alignItems:"flex-end",gap:8}} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>{setHov(false);setShowR(false);}}>
      ...
      <div style={{maxWidth:"75%",minWidth:100,position:"relative"}}>
        ...
        <div style={{background:isSelf?"linear-gradient(135deg,#0071e3,#0055b0)":tc?tc.bg:"#1e293b",border:`1px solid ${isSelf?"#0060c0":tc?tc.bd:"rgba(255,255,255,0.1)"}`,borderRadius:isSelf?"16px 16px 4px 16px":"16px 16px 16px 4px",padding:"8px 12px"}}>
          {tc&&<div style={{fontSize:10,fontWeight:700,color:"#94a3b8",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.5px"}}>{tc.ic} {tc.lb}</div>}
          {!isSelf&&sender&&<div style={{fontSize:12,fontWeight:700,color:sender.color,marginBottom:3}}>{sender.name} {sender.emoji}</div>}
          {replyMsg&&<div style={{padding:"4px 8px",marginBottom:6,borderLeft:`3px solid ${members[replyMsg.sender]?.color||"#0071e3"}`,background:"rgba(255,255,255,0.05)",borderRadius:"0 6px 6px 0",fontSize:11,color:"#94a3b8"}}><span style={{color:members[replyMsg.sender]?.color,fontWeight:600}}>{members[replyMsg.sender]?.name}</span><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:220}}>{replyMsg.text}</div></div>}
          {msg.type===CT.POLL&&msg.pollOpts?<div><div style={{fontSize:14,color:isSelf?"#fff":"#e2e8f0",marginBottom:8,fontWeight:600}}>{msg.text}</div>{msg.pollOpts.map((o,i)=>{const tot=msg.pollOpts.reduce((s,x)=>s+x.votes.length,0);const pct=tot>0?(o.votes.length/tot)*100:0;return<div key={i} style={{marginBottom:6,position:"relative",borderRadius:8,overflow:"hidden",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.06)"}}><div style={{position:"absolute",top:0,left:0,height:"100%",width:`${pct}%`,background:"rgba(0,113,227,0.1)"}} /><div style={{position:"relative",padding:"6px 10px",display:"flex",justifyContent:"space-between",fontSize:13,color:"#f1f5f9"}}><span>{o.text}</span><span style={{color:"#0071e3",fontWeight:600}}>{o.votes.length} ({Math.round(pct)}%)</span></div></div>;})}</div>
          :<div style={{fontSize:14,color:isSelf?"#fff":"#e2e8f0",whiteSpace:"pre-wrap",lineHeight:1.45}}>{msg.text}</div>}
          <div style={{fontSize:10,color:isSelf?"rgba(255,255,255,0.6)":"#64748b",marginTop:4,textAlign:"right"}}>{msg.time}{isSelf&&" ✓✓"}</div>
        </div>
        {Object.keys(msg.reactions).length>0&&<div style={{display:"flex",gap:4,marginTop:2,flexWrap:"wrap",justifyContent:isSelf?"flex-end":"flex-start"}}>
          {Object.entries(msg.reactions).map(([em,us])=><span key={em} onClick={()=>onReact(msg.id,em)} style={{background:us.includes(self)?"rgba(0,113,227,0.2)":"rgba(30,41,59,0.6)",border:`1px solid ${us.includes(self)?"rgba(0,113,227,0.35)":"rgba(255,255,255,0.1)"}`,borderRadius:12,padding:"1px 6px",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",gap:3}}>{em}<span style={{fontSize:10,color:"#94a3b8"}}>{us.length}</span></span>)}
        </div>}
      </div>
    </div>
  );
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)"}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#111827",borderRadius:20,padding:28,width:360,maxWidth:"90vw",boxShadow:"0 20px 60px rgba(0,0,0,0.15)"}}>
        ...
        <div style={{marginBottom:12}}><div style={{fontSize:12,color:"#94a3b8",marginBottom:6}}>Color</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{cols.map(c=><div key={c} onClick={()=>setCol(c)} style={{width:28,height:28,borderRadius:"50%",background:c,cursor:"pointer",border:col===c?"3px solid #60a5fa":"3px solid transparent"}} />)}</div></div>
        <div style={{marginBottom:20}}><div style={{fontSize:12,color:"#94a3b8",marginBottom:6}}>Avatar</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{emos.map(e=><div key={e} onClick={()=>setEmo(e)} style={{width:36,height:36,borderRadius:8,background:emo===e?"rgba(0,113,227,0.2)":"rgba(30,41,59,0.6)",border:emo===e?"2px solid #0071e3":"2px solid transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,cursor:"pointer"}}>{e}</div>)}</div></div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{flex:1,padding:"10px",borderRadius:10,border:"1.5px solid rgba(255,255,255,0.1)",background:"transparent",color:"#94a3b8",cursor:"pointer",fontSize:14}}>Cancel</button>
...
export default function App() {
  ...
  const PAGE_TITLES = {
    ...
    topics: "730+ Mainframe Tutorials — JCL, COBOL, DB2, CICS, VSAM | MainframeStudyHub",
  ...
  const PAGE_DESCS = {
    home: "MainframeStudyHub — Free IBM Z mainframe learning platform. 730+ lessons across 15 topics. AI Code Lab, 200+ quizzes, interview Q&A, cheat sheets.",
    topics: "730+ mainframe tutorials: JCL (86), COBOL (81), DB2 (61), CICS (61), VSAM (61), REXX, IMS, z/OS, RACF, TSO, SMF, DFSORT. Beginner to Expert. Free., REXX, IMS, z/OS, RACF, TSO, SMF, DFSORT. Beginner to Expert. Free.",
  ...
  const renderContent = (text) => {
    ...
    while (i < lines.length) {
      ...
      if (line.match(/^[A-Z][A-Z0-9\/\-_]{1,20}\s*(—|–|-|=|:)\s/) && line.length < 200) {
        ...
        if (sepMatch) {
          elements.push(
            <div key={i} style={{ margin:"6px 0", padding:"10px 16px", background:"rgba(30,41,59,0.6)",
              ...
              <span style={{ color:"#94a3b8" }}> {sepMatch[2]} </span>
      ...
      // Regular paragraph
      elements.push(
        <p key={i} style={{ fontSize:15.5, color:"#cbd5e1", lineHeight:2, margin:"8px 0",
  ...
  return (
    <div style={S.root}>
      ...
      <nav role="navigation" aria-label="Main navigation" style={{ ...S.nav,
        background: scrolled ? "rgba(8,11,22,0.95)" : "rgba(8,11,22,0.8)",
        ...
        <div style={S.navInner}>
          ...
          <button aria-label="Menu" style={S.hamburger} onClick={() => setNavOpen(o => !o)}>
            <div style={{ width:18,height:1.5,background:"#e2e8f0",marginBottom:5,transform:navOpen?"rotate(45deg) translate(4px,4px)":"none",transition:"all .2s" }} />
            <div style={{ width:18,height:1.5,background:"#e2e8f0",marginBottom:5,opacity:navOpen?0:1,transition:"all .2s" }} />
            <div style={{ width:18,height:1.5,background:"#e2e8f0",transform:navOpen?"rotate(-45deg) translate(4px,-4px)":"none",transition:"all .2s" }} />
          </button>
        </div>
      </nav>
          ...
          {user ? (
            <div style={{ padding:"12px 24px",display:"flex",alignItems:"center",gap:12,borderBottom:"1px solid rgba(255,255,255,0.06)",marginBottom:8 }}>
              ...
              <div style={{ flex:1,minWidth:0 }}>
                ...
                <div style={{ fontSize:11,color:"#94a3b8" }}>{user.role} · {user.mfYears}yr MF</div>
              </div>
      ...
      <main id="main-content" role="main" style={{ paddingTop: 52, minHeight: "100vh" }}>
          ...
          <div style={{ position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",
            ...
            <div className="scaleIn" style={{ background:"rgba(8,11,22,0.97)",backdropFilter:"blur(20px)",borderRadius:24,
              ...
              <button onClick={authSignInWithGoogle} disabled={authLoading}
                style={{ width:"100%",padding:"11px",background:"#1e293b",color:"#f1f5f9",border:"1.5px solid rgba(255,255,255,0.15)",
                ...
                onMouseEnter={e => { e.currentTarget.style.background="rgba(30,41,59,0.8)"; e.currentTarget.style.boxShadow="0 2px 8px rgba(0,0,0,0.08)"; }}
                onMouseLeave={e => { e.currentTarget.style.background="rgba(30,41,59,0.5)"; e.currentTarget.style.boxShadow="none"; }}>
              ...
              {authModal==="signup" && (
                <>
                  <div style={{ marginBottom:16 }}>
                    <label style={{ fontSize:12,fontWeight:600,color:"#94a3b8",display:"block",marginBottom:4 }}>Current Role</label>
                    <select value={authForm.role} onChange={e => setAuthForm({...authForm, role:e.target.value})}
                      style={{ ...modalInput, marginBottom:0,cursor:"pointer",color:authForm.role?"#f1f5f9":"#64748b" }}>
                  ...
                  <div style={{ display:"flex",gap:10,marginBottom:20 }}>
                    <div style={{ flex:1 }}>
                      <label style={{ fontSize:12,fontWeight:600,color:"#94a3b8",display:"block",marginBottom:4 }}>IT Experience (years)</label>
                    ...
                    <div style={{ flex:1 }}>
                      <label style={{ fontSize:12,fontWeight:600,color:"#94a3b8",display:"block",marginBottom:4 }}>Mainframe Exp (years)</label>
              ...
              <button onClick={authModal==="signin" ? authSignIn : authSignUp}
                ...
                className="glow-btn"
                style={{ width:"100%",padding:"12px",background:authLoading?"#334155":"linear-gradient(135deg,#0071e3,#7c3aed)",color:"#fff",
              ...
              {authModal==="signin" && (
                <div style={{ textAlign:"center",marginTop:10 }}>
                  <button onClick={authForgotPassword}
                    style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontFamily:FF,fontSize:12,fontWeight:500 }}>
            ...
            <div className="scaleIn" style={{ position:"fixed",top:48,right:16,zIndex:9999,background:"rgba(8,11,22,0.97)",
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:18 }}>
                <div style={{ background:"rgba(30,41,59,0.8)",borderRadius:12,padding:"12px",textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:10,color:"#94a3b8",fontWeight:600 }}>IT YEARS</div>
                </div>
                <div style={{ background:"rgba(30,41,59,0.8)",borderRadius:12,padding:"12px",textAlign:"center" }}>
                  <div style={{ fontSize:22,fontWeight:800,color:"#f1f5f9" }}>{user?.mfYears}</div>
                  <div style={{ fontSize:10,color:"#94a3b8",fontWeight:600 }}>MF YEARS</div>
                </div>
              </div>
            ...
            <div className="scaleIn" style={{ background:"rgba(8,11,22,0.97)",backdropFilter:"blur(20px)",borderRadius:24,
              ...
              <div style={{ display:"flex",gap:10 }}>
                <button onClick={saveProfile} disabled={authLoading} className="glow-btn"
                  style={{ flex:1,padding:"12px",background:authLoading?"#334155":"linear-gradient(135deg,#0071e3,#7c3aed)",color:"#fff",
        ...
        {page === "home" && (
          <div>
                ...
                <div style={{ display:"flex",justifyContent:"center",gap:0,flexWrap:"wrap",
                  ...
                  {[["15","Topics","📚"],["192+","Sections","📄"],["200","Quiz Qs","🧠"],["87","Abend Codes","🔍"],["6","Levels","🗺️"],["Weekly","AI Updates","🤖"]].map(([n,l,icon],i) => (
                    <div key={l} className="fu stat-card" style={{ flex:"1 1 120px",textAlign:"center",padding:"12px 8px",
                      ...
                      <div style={{ fontSize:11,color:"#94a3b8",fontWeight:600,marginTop:4 }}>{l}</div>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={S.topicsGrid}>
                  {TOPICS.map((t,i) => (
                    <button key={t.id} className="card fu" style={{ ...S.topicCard, borderTop:`3px solid ${t.color}`, animationDelay:`${i*25}ms` }}
                      ...
                      <div style={{ fontSize:11,color:"#94a3b8",marginBottom:10 }}>{t.level}</div>
            ...
            <section style={{ padding:"72px 0", background:"linear-gradient(180deg, rgba(8,11,22,0.95) 0%, rgba(12,18,32,0.95) 100%)" }}>
          ...
          <div>
            {!activeTopic ? (
              <div>
                ...
                <div style={{ borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"14px 0",position:"sticky",top:52,background:"rgba(8,11,22,0.95)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",zIndex:100 }}>
                  <div style={{ ...S.inner,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap" }}>
                    <div style={{ position:"relative",flexShrink:0 }}>
                      <span style={{ position:"absolute",left:11,top:"50%",transform:"translateY(-50%)",color:"#64748b",fontSize:14 }}>⌕</span>
                    ...
                    <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
                      {categories.map(c => (
                        <button key={c.id} className="pill"
                          style={{ ...S.pill, background:cat===c.id?"#0071e3":"rgba(30,41,59,0.8)", color:cat===c.id?"#fff":"#94a3b8" }}
              ...
              <div className="fi">
                ...
                <div className="topic-layout" style={{ ...S.inner, display:"flex", gap:0, alignItems:"flex-start", paddingTop:0, paddingBottom:60 }}>
                  ...
                  <div className="topic-mobile-nav" style={{ width:"100%",padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.06)" }}>
                  ...
                  {/* ── MAIN CONTENT ── */}
                  <div className="topic-content-main scaleIn" key={activeTab}>
                    {/* Breadcrumb */}
                      ...
                      {activeTopic.sections[activeTab].code && (
                        <div style={S.codeWrap}>
                          <div style={S.codeTopBar}>
                            ...
                            <span style={{ fontSize:11,color:"#94a3b8",letterSpacing:"1px" }}>CODE EXAMPLE</span>
                          </div>
                    ...
                    <div style={{ display:"flex",gap:12,marginTop:40,paddingTop:24,borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                      {activeTab > 0 && (
                        <button onClick={() => { setActiveTab(activeTab-1); window.scrollTo(0,280); }}
                          ...
                          <div style={{ fontSize:11,color:"#64748b",marginBottom:4 }}>‹ Previous Lesson</div>
                      ...
                      {activeTab < activeTopic.sections.length - 1 && (
                        <button onClick={() => { setActiveTab(activeTab+1); window.scrollTo(0,280); }}
                          ...
                          <div style={{ fontSize:11,color:"#64748b",marginBottom:4 }}>Next Lesson ›</div>
                ...
                <div style={{ ...S.inner,paddingBottom:60,borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:28,display:"flex",gap:16,flexWrap:"wrap" }}>
                  {TOPICS[TOPICS.indexOf(activeTopic)-1] && (
                    <button style={S.prevNextBtn} onClick={() => openTopic(TOPICS[TOPICS.indexOf(activeTopic)-1])}>
                      <span style={{ fontSize:11,color:"#94a3b8",display:"block",marginBottom:4 }}>Previous</span>
                  ...
                  {TOPICS[TOPICS.indexOf(activeTopic)+1] && (
                    <button style={{ ...S.prevNextBtn,textAlign:"right",marginLeft:"auto" }} onClick={() => openTopic(TOPICS[TOPICS.indexOf(activeTopic)+1])}>
                      <span style={{ fontSize:11,color:"#94a3b8",display:"block",marginBottom:4 }}>Next</span>
                      <span style={{ fontSize:15,fontWeight:500 }}>{TOPICS[TOPICS.indexOf(activeTopic)+1].icon} {TOPICS[TOPICS.indexOf(activeTopic)+1].title}</span>
                    </button>
                  )}
                </div>
              ...
              <div style={{ ...S.inner,display:"flex",gap:10,flexWrap:"wrap",alignItems:"center" }}>
                ...
                {scenCats.map(c => (
                  <button key={c} className="pill"
                    style={{ ...S.pill,fontSize:11,padding:"5px 12px",background:scenarioCat===c?"#0071e3":"rgba(30,41,59,0.8)",color:scenarioCat===c?"#fff":"#94a3b8" }}
                    onClick={() => setScenarioCat(c)}>{c}</button>
                ))}
                <span style={{ fontSize:12,color:"#94a3b8",fontWeight:500,marginLeft:8 }}>Level:</span>
                {scenDiffs.map(d => (
                  <button key={d} className="pill"
                    style={{ ...S.pill,fontSize:11,padding:"5px 12px",background:scenarioDiff===d?"#0071e3":"rgba(255,255,255,0.1)",color:scenarioDiff===d?"#fff":"#94a3b8" }}
          ...
          <div>
            <div style={S.pageHero}>
              <div style={{ display:"flex",alignItems:"flex-start",justifyContent:"space-between",flexWrap:"wrap",gap:16 }}>
                ...
                {user && !canWriteBlog && (
                  <div style={{ fontSize:12,color:"#94a3b8",background:"rgba(30,41,59,0.8)",borderRadius:10,padding:"8px 14px",maxWidth:220,marginTop:8 }}>
                ...
                <div className="scaleIn" style={{ background:"rgba(17,24,39,0.9)",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.08)",
                  ...
                  <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:20 }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:12,color:"#94a3b8" }}>{user.role} · {user.mfYears} yrs mainframe experience</div>
                    </div>
                  ...
                  <input value={blogDraft.title} onChange={e => setBlogDraft({...blogDraft, title:e.target.value})}
                    ...
                    style={{ width:"100%",padding:"14px 16px",fontSize:18,fontWeight:700,border:"1.5px solid rgba(255,255,255,0.1)",
                      borderRadius:12,outline:"none",fontFamily:FF,background:"rgba(30,41,59,0.6)",marginBottom:14,color:"#f1f5f9" }} />
                  <div style={{ display:"flex",gap:10,marginBottom:14 }}>
                    <select value={blogDraft.category} onChange={e => setBlogDraft({...blogDraft, category:e.target.value})}
                      style={{ padding:"8px 14px",fontSize:13,border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:10,
                        outline:"none",fontFamily:FF,background:"rgba(30,41,59,0.6)",cursor:"pointer",color:"#f1f5f9" }}>
                  ...
                  <textarea value={blogDraft.content} onChange={e => setBlogDraft({...blogDraft, content:e.target.value})}
                    ...
                    style={{ width:"100%",padding:"16px",fontSize:15,lineHeight:1.8,border:"1.5px solid rgba(255,255,255,0.1)",
                      borderRadius:12,outline:"none",fontFamily:FF,background:"rgba(30,41,59,0.6)",resize:"vertical",
                      minHeight:200,color:"#cbd5e1" }} />
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(320px,1fr))",gap:24 }}>
                  {allBlogs.map((b,i) => (
                    <button key={b.id} className="card fu" onClick={() => setExpandedBlog(b.id)}
                      ...
                      <p style={{ fontSize:13,color:"#94a3b8",marginBottom:16,textAlign:"left" }}>
                        {b.content.substring(0,150)}…
                      </p>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                        <div style={{ fontSize:12,color:"#94a3b8",textAlign:"left" }}>
                      ...
                      <div>
                        ...
                        {b.isUserBlog && (
                          <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:28,padding:"16px 20px",
                            background:"rgba(30,41,59,0.7)",borderRadius:14,border:"1px solid rgba(255,255,255,0.06)" }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize:12,color:"#94a3b8" }}>{b.authorRole} · {b.authorMfYears} years mainframe experience</div>
                            </div>
          ...
          <div>
            ...
            <div style={{ ...S.inner,paddingBottom:80 }}>
              ...
              {dailyMode && !quiz.done && (
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,
                  ...
                  <button onClick={() => { setDailyMode(false); clearInterval(dailyTimerRef.current); setQuiz({ index:0,score:0,selected:null,done:false,showExp:false }); }}
                    style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:13,fontFamily:FF }}>✕ Exit Challenge</button>
                </div>
              )}
                ...
                {QUIZ_TOPICS.map(t => (
                  <button key={t} onClick={() => { setQuizTopicFilter(t); setQuiz({ index:0,score:0,selected:null,done:false,showExp:false }); }}
                    style={{ padding:"6px 14px",borderRadius:980,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:FF,border:"none",
                      background:quizTopicFilter===t?"linear-gradient(135deg,#0071e3,#7c3aed)":"rgba(30,41,59,0.8)",
                      color:quizTopicFilter===t?"#fff":"#94a3b8",transition:"all .15s" }}>
              ...
              <div style={{ maxWidth:640,margin:"0 auto" }}>
                {activeQs.length === 0 ? (
                  <div style={{ textAlign:"center",padding:60,color:"#94a3b8" }}>No questions for this topic yet.</div>
                ) : !quiz.done ? (
          ...
          <div>
            <div style={S.pageHero}>
              ...
              <p style={S.pageHeroSub}>
                ...
                {lastUpdate && <span style={{ display:"block",fontSize:13,color:"#94a3b8",marginTop:6 }}>Last updated: {new Date(lastUpdate).toLocaleDateString()}</span>}
              </p>
            </div>
            <div style={{ ...S.inner,paddingBottom:80 }}>
              {/* Topic picker */}
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10 }}>
                  {TOPICS.map(t => (
                    <button key={t.id} onClick={() => fetchUpdate(t)}
                      style={{ background:weeklyTopic?.id===t.id?"#0071e3":"rgba(30,41,59,0.8)",
                        color:weeklyTopic?.id===t.id?"#fff":"#94a3b8",
              ...
              {weeklyLoading && (
                <div style={{ textAlign:"center",padding:"60px 0" }}>
                  <div style={{ width:40,height:40,border:"3px solid rgba(255,255,255,0.1)",borderTop:"3px solid #0071e3",borderRadius:"50%",animation:"spin 0.8s linear infinite",margin:"0 auto 16px" }} />
                  <div style={{ fontSize:16,color:"#94a3b8" }}>Fetching fresh content for {weeklyTopic?.title}…</div>
                  <div style={{ fontSize:13,color:"#94a3b8",marginTop:6 }}>This may take a moment</div>
                </div>
              )}
                ...
                <div className="fi">
                  <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:12,marginBottom:24 }}>
                    <div>
                      ...
                      <div style={{ fontSize:13,color:"#94a3b8",marginTop:4 }}>Generated: {weeklyUpdate.generatedDate}</div>
                    </div>
                  ...
                  <div style={{ display:"flex",gap:4,marginBottom:24,flexWrap:"wrap" }}>
                    {["tip","scenario","code","facts"].map(tab => (
                      <button key={tab} onClick={() => setWeeklyTab(tab)}
                        style={{ background:weeklyTab===tab?"#0071e3":"rgba(30,41,59,0.8)",color:weeklyTab===tab?"#fff":"#94a3b8",
          ...
          <div>
            ...
            <div style={{ ...S.inner, paddingTop:24, paddingBottom:80 }}>
              ...
              <div style={{ display:"flex",gap:10,flexWrap:"wrap",marginBottom:16,alignItems:"center" }}>
                {["JCL","COBOL","REXX","DB2 SQL"].map(lang => (
                  <button key={lang} onClick={() => { setPgLang(lang); setPgCode(pgSamples[lang]); setPgResult(null); }}
                    style={{ padding:"8px 16px",borderRadius:10,border:pgLang===lang?"2px solid #0071e3":"2px solid rgba(255,255,255,0.12)",
                      background:pgLang===lang?"rgba(0,113,227,0.15)":"rgba(30,41,59,0.6)",color:pgLang===lang?"#60a5fa":"#94a3b8",
                ...
                <div style={{ marginLeft:"auto",display:"flex",gap:6 }}>
                  {[["explain","💡 Explain"],["errors","🔍 Find Errors"],["simulate","▶️ Simulate"]].map(([m,label]) => (
                    <button key={m} onClick={() => setPgMode(m)}
                      style={{ padding:"8px 14px",borderRadius:10,border:pgMode===m?"2px solid #7c3aed":"2px solid rgba(255,255,255,0.12)",
                        background:pgMode===m?"rgba(124,58,237,0.15)":"rgba(30,41,59,0.6)",color:pgMode===m?"#a78bfa":"#94a3b8",
              ...
              <div style={{ display:"flex",gap:8,marginTop:12,flexWrap:"wrap" }}>
                <span style={{ fontSize:12,color:"#64748b",lineHeight:"32px" }}>Try:</span>
                {[
              ...
              {pgLoading && (
                <div style={{ marginTop:24,textAlign:"center",padding:"40px 0" }}>
                  <div style={{ width:40,height:40,border:"3px solid rgba(255,255,255,0.12)",borderTop:"3px solid #0071e3",
          ...
          <div>
            ...
            <div style={S.pageHero}>
              ...
              <div style={{ display:"flex",gap:8,marginTop:20 }}>
                <button onClick={()=>setCommunityView("chat")} style={{ padding:"10px 24px",borderRadius:980,border:communityView==="chat"||communityView===null?"none":"1.5px solid rgba(255,255,255,0.12)",background:communityView==="chat"||communityView===null?"#0071e3":"rgba(30,41,59,0.8)",color:communityView==="chat"||communityView===null?"#fff":"#94a3b8",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:FF }}>🖥️ Group Chat</button>
                <button onClick={()=>setCommunityView("qa")} style={{ padding:"10px 24px",borderRadius:980,border:communityView==="qa"?"none":"1.5px solid rgba(255,255,255,0.12)",background:communityView==="qa"?"#0071e3":"rgba(30,41,59,0.8)",color:communityView==="qa"?"#fff":"#94a3b8",fontSize:14,fontWeight:600,cursor:"pointer",fontFamily:FF }}>❓ Q&A Forum</button>
              </div>
            </div>
                  ...
                  <div style={{ border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:20, overflow:"hidden", boxShadow:"0 4px 20px rgba(0,0,0,0.3)", height:"70vh", display:"flex" }}>
                    ...
                    <div style={{ width:chatSidebar?260:0, minWidth:chatSidebar?260:0, background:"#0f172a", borderRight:"1px solid rgba(255,255,255,0.12)", transition:"all 0.3s", overflow:"hidden", display:"flex", flexDirection:"column" }}>
                      <div style={{ padding:"12px 16px", borderBottom:"1px solid rgba(255,255,255,0.06)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                        ...
                        <div style={{ display:"flex",gap:6 }}>
                          ...
                          <button onClick={()=>setChatSidebar(false)} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:14 }}aria-label="Close">✕</button>
                        </div>
                      </div>
                    ...
                    <div style={{ flex:1,display:"flex",flexDirection:"column",minWidth:0 }}>
                      ...
                      <div style={{ padding:"10px 16px",background:"#111827",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",gap:10 }}>
                        <button onClick={()=>setChatSidebar(!chatSidebar)} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:16,padding:2 }}aria-label="Toggle menu">☰</button>
                        <div style={{ width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,rgba(0,113,227,0.15),rgba(0,113,227,0.08))",border:"1px solid rgba(0,113,227,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18 }}>🖥️</div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:14,fontWeight:700,color:"#f1f5f9" }}>MainframeStudyHub</div>
                          <div style={{ fontSize:11,color:"#94a3b8" }}>{chatMembers.length} members • {chatOnline} online</div>
                        </div>
                        <button onClick={()=>{setChatShowSrch(!chatShowSrch);setChatSrch("");}} style={{ background:chatShowSrch?"rgba(0,113,227,0.15)":"transparent",border:"none",borderRadius:6,color:chatShowSrch?"#60a5fa":"#94a3b8",cursor:"pointer",fontSize:14,padding:"4px 8px" }}aria-label="Search">🔍</button>
                        <button onClick={()=>setChatShowStars(!chatShowStars)} aria-label="Toggle bookmarks" style={{ background:chatShowStars?"rgba(212,160,23,0.15)":"transparent",border:"none",borderRadius:6,color:chatShowStars?"#d4a017":"#94a3b8",cursor:"pointer",fontSize:14,padding:"4px 8px" }}>{chatShowStars?"⭐":"☆"}</button>
                      </div>
                      {chatShowSrch&&<div style={{ padding:"6px 16px",background:"#1e293b",borderBottom:"1px solid rgba(255,255,255,0.12)" }}><input value={chatSrch} onChange={e=>setChatSrch(e.target.value)} placeholder="Search messages..." autoFocus style={{ width:"100%",boxSizing:"border-box",padding:"6px 12px",borderRadius:8,border:"1.5px solid rgba(255,255,255,0.1)",background:"#111827",color:"#f1f5f9",fontSize:13,outline:"none",fontFamily:FF }} /></div>}
                      ...
                      <div style={{ flex:1,overflowY:"auto",padding:"12px 0",background:"#0f172a" }}>
                        ...
                        {chatFiltered.length===0&&<div style={{ textAlign:"center",padding:40,color:"#94a3b8",fontSize:13 }}>{chatShowStars?"No starred messages":"No messages found"}</div>}
                        <div ref={chatEnd} />
                      </div>
                      {/* Reply */}
                      {chatReply&&<div style={{ padding:"6px 16px",background:"#1e293b",borderTop:"1px solid rgba(255,255,255,0.12)",display:"flex",alignItems:"center",gap:8 }}>
                        <div style={{ width:3,height:24,borderRadius:2,background:chatMembers[chatReply.sender]?.color||"#0071e3" }} />
                        <div style={{ flex:1,minWidth:0 }}><div style={{ fontSize:11,color:chatMembers[chatReply.sender]?.color,fontWeight:600 }}>{chatMembers[chatReply.sender]?.name}</div><div style={{ fontSize:11,color:"#94a3b8",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{chatReply.text}</div></div>
                        <button onClick={()=>setChatReply(null)} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:14 }}aria-label="Close">✕</button>
                      </div>}
                      {/* Input */}
                      <div style={{ padding:"8px 12px",background:"#111827",borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                        <div style={{ display:"flex",gap:4,marginBottom:6 }}>
                          {[{t:CT.TEXT,i:"💬",l:"Message"},{t:CT.JOB,i:"💼",l:"Job"},{t:CT.DOUBT,i:"❓",l:"Doubt"},{t:CT.THOUGHT,i:"💭",l:"Thought"}].map(x=>
                            <button key={x.t} onClick={()=>setChatMsgType(x.t)} style={{ padding:"3px 10px",borderRadius:980,border:`1.5px solid ${chatMsgType===x.t?"#0071e3":"rgba(255,255,255,0.1)"}`,background:chatMsgType===x.t?"rgba(0,113,227,0.15)":"rgba(30,41,59,0.6)",color:chatMsgType===x.t?"#60a5fa":"#94a3b8",fontSize:11,cursor:"pointer",fontFamily:FF,display:"flex",alignItems:"center",gap:3 }}>{x.i} {x.l}</button>
                          )}
                        </div>
                        <div style={{ display:"flex",gap:8,alignItems:"flex-end" }}>
                          ...
                          <button onClick={chatSend} disabled={!grpInput.trim()} style={{ width:38,height:38,borderRadius:10,border:"none",background:grpInput.trim()?"#0071e3":"rgba(255,255,255,0.1)",color:grpInput.trim()?"#fff":"#64748b",fontSize:16,cursor:grpInput.trim()?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>➤</button>
                        </div>
                      </div>
                    </div>
                  ...
                  <div style={{ display:"flex",gap:4 }}>
                    {["hot","new","top"].map(s => (
                      <button key={s} onClick={() => setCommunitySort(s)}
                        style={{ ...S.pill, background:communitySort===s?"#0071e3":"rgba(30,41,59,0.8)", color:communitySort===s?"#fff":"#94a3b8",
                          border:communitySort===s?"none":"1.5px solid rgba(255,255,255,0.12)",textTransform:"capitalize" }}>{s==="hot"?"🔥 Hot":s==="new"?"🕐 New":"⬆️ Top"}</button>
                    ))}
                  </div>
                ...
                {newPostOpen && (
                  <div className="fi" style={{ border:"1.5px solid #0071e3",borderRadius:16,padding:20,marginBottom:24,background:"#0f172a" }}>
                    ...
                    <div style={{ display:"flex",gap:10,alignItems:"center" }}>
                      ...
                      <button onClick={() => setNewPostOpen(false)} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:13,fontFamily:FF }}>Cancel</button>
                    </div>
                  </div>
                )}
                  ...
                  <div key={post.id} className="card" style={{ border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:16, padding:"20px 24px", marginBottom:16, cursor:"pointer", boxShadow:"0 2px 8px rgba(0,0,0,0.2)" }}
                    ...
                    <div style={{ display:"flex",gap:16,alignItems:"flex-start" }}>
                      <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:4,minWidth:40 }}>
                        <button onClick={e => { e.stopPropagation(); votePost(post.id, 1); }} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:16 }}>▲</button>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ display:"flex",gap:10,alignItems:"center",fontSize:12,color:"#94a3b8" }}>
                ...
                <div style={{ ...S.inner, maxWidth:800, paddingBottom:80 }}>
                  ...
                  <div style={{ border:"1.5px solid rgba(255,255,255,0.1)", borderRadius:20, padding:"28px 32px", marginBottom:32, background:"#111827" }}>
                    <div style={{ display:"flex",gap:16 }}>
                      <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
                        <button onClick={() => votePost(post.id, 1)} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:20 }}>▲</button>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:13,color:"#94a3b8" }}>Asked by <strong style={{ color:"#f1f5f9" }}>{post.author}</strong>{post.authorRole && <span style={{ color:"#0071e3" }}> • {post.authorRole}</span>} on {post.date}</div>
                      </div>
                    </div>
                  </div>
                  ...
                  {post.answers.map(ans => (
                    <div key={ans.id} style={{ border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"20px 24px",marginBottom:16,background:"#111827" }}>
                      <div style={{ display:"flex",gap:12 }}>
                        <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:4 }}>
                          <button onClick={() => voteAnswer(post.id, ans.id, 1)} style={{ background:"none",border:"none",color:"#94a3b8",cursor:"pointer",fontSize:16 }}>▲</button>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <div style={{ fontSize:12,color:"#94a3b8",marginTop:10 }}>Answered by <strong style={{ color:"#f1f5f9" }}>{ans.author}</strong>{ans.authorRole && <span style={{ color:"#0071e3" }}> • {ans.authorRole}</span>} on {ans.date}</div>
                        </div>
                      </div>
                    </div>
                  ))}
                  {user ? (
                    ...
                    <div style={{ border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:16,padding:"24px 20px",textAlign:"center",background:"#0f172a" }}>
                      <p style={{ color:"#94a3b8",marginBottom:12 }}>Sign in to post your answer</p>
            ...
            <div style={{ ...S.inner, paddingBottom:80 }}>
              ...
              <div className="content-card fi" style={{ marginBottom:24 }}>
                <div style={{ display:"flex",gap:12,flexWrap:"wrap",alignItems:"center",marginBottom:18 }}>
                  ...
                  <div style={{ fontSize:13,color:"#94a3b8",fontWeight:600 }}>{filteredAbends.length} codes</div>
                </div>
                <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
                  {ABEND_CATEGORIES.map(cat => (
                    <button key={cat} onClick={() => { setAbendCategory(cat); setAbendExpanded(null); }}
                      style={{ padding:"6px 16px",borderRadius:980,border:"1.5px solid",fontSize:12,fontWeight:600,
                        ...
                        color:abendCategory===cat?"#fff":"#94a3b8",
                    ...
                    <div onClick={() => setAbendExpanded(abendExpanded===a.code?null:a.code)}
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" }}>
                          ...
                          <span style={{ fontSize:11,color:"#94a3b8",background:"rgba(30,41,59,0.8)",padding:"2px 8px",borderRadius:980 }}>{a.category}</span>
                        </div>
                      </div>
                      <span style={{ fontSize:18,color:"#94a3b8",transition:"transform 0.3s",
                      ...
                      <div style={{ display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,marginBottom:roadmapLevel===lvl.level?16:0 }}>
                        <div>
                          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:4 }}>
                            ...
                            <span style={{ fontSize:11,color:"#94a3b8" }}>{lvl.duration}</span>
                          </div>
                          <div style={{ fontSize:20,fontWeight:800,color:"#f1f5f9",letterSpacing:"-0.3px" }}>
                            {lvl.icon} {lvl.title}
                          </div>
                          <div style={{ fontSize:13,color:"#94a3b8" }}>{lvl.subtitle}</div>
                        </div>
                        <span style={{ fontSize:18,color:"#94a3b8",transition:"transform 0.3s",
              ...
              <div className="content-card fi" style={{ marginBottom:32 }}>
                <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:28 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize:14,color:"#94a3b8" }}>Built with passion for the Mainframe community</p>
                  </div>
                </div>
              ...
              <div className="content-card fi" style={{ marginBottom:32,animationDelay:"0.1s" }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:14 }}>
                  {[
                    ...
                    <div key={i} style={{ background:"rgba(30,41,59,0.6)",borderRadius:14,padding:"18px 16px",
            ...
            {welcomePhase === 2 && (
              <div>
                ...
                <div style={{ padding:"20px 28px 28px",textAlign:"center" }}>
                  ...
                  <div style={{ display:"flex",gap:10 }}>
                    <button onClick={()=>setWelcomePhase(0)} style={{ flex:1,padding:"12px",borderRadius:12,border:"1.5px solid rgba(255,255,255,0.1)",background:"transparent",color:"#94a3b8",cursor:"pointer",fontSize:14,fontFamily:FF }}>Maybe Later</button>
            ...
            {welcomePhase === 3 && (
              <div>
                ...
                <div style={{ padding:"20px 28px 28px",textAlign:"center" }}>
                  ...
                  <div style={{ display:"flex",gap:10 }}>
                    <button onClick={()=>setWelcomePhase(0)} style={{ flex:1,padding:"12px",borderRadius:12,border:"1.5px solid rgba(255,255,255,0.1)",background:"transparent",color:"#94a3b8",cursor:"pointer",fontSize:14,fontFamily:FF }}>Later</button>
          ...
          <div onClick={e=>e.stopPropagation()} className="scaleIn" style={{
            ...
            {feedbackSent ? (
              <div style={{ textAlign:"center",padding:"20px 0" }}>
                ...
                <p style={{ fontSize:14,color:"#94a3b8" }}>Your feedback helps us improve.</p>
              </div>
            ) : (
              <>
                <div style={{ textAlign:"center",marginBottom:20 }}>
                  ...
                  <p style={{ fontSize:13,color:"#94a3b8" }}>We'd love your feedback to make MainframeStudyHub even better</p>
                </div>
                {/* Star Rating */}
                ...
                <div style={{ display:"flex",gap:10 }}>
                  <button onClick={()=>{ setFeedbackOpen(false); localStorage.setItem("mfsh_feedback_done","1"); }}
                    style={{ flex:1,padding:"12px",borderRadius:12,border:"1.5px solid rgba(255,255,255,0.1)",background:"transparent",
                      color:"#94a3b8",cursor:"pointer",fontSize:14,fontFamily:FF }}>Maybe Later</button>
        ...
        <div style={{ position:"fixed",inset:0,zIndex:2500,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(10px)",display:"flex",alignItems:"center",justifyContent:"center" }} onClick={()=>setChatPopup(false)}>
          <div onClick={e=>e.stopPropagation()} style={{
            ...
            <div style={{ padding:"0 26px 26px",opacity:chatPopPhase>=3?1:0,transform:chatPopPhase>=3?"translateY(0)":"translateY(10px)",transition:"all 0.4s ease 0.4s" }}>
              ...
              <p style={{ fontSize:11,color:"#94a3b8",marginTop:8,textAlign:"center" }}>New here? Create a free account</p>
          ...
          <div style={{ flex:1,overflowY:"auto",padding:chatMax?"24px 0":"16px 18px",display:"flex",flexDirection:"column",gap:12,
            ...
            {chatLoading && (
              <div style={{ display:"flex",gap:10 }}>
                <div style={{ width:30,height:30,borderRadius:10,background:"linear-gradient(135deg,#0071e3,#7c3aed)",
                  color:"#fff",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>🤖</div>
                <div style={{ padding:"14px 20px",background:"rgba(30,41,59,0.9)",borderRadius:"18px 18px 18px 4px",
                  display:"flex",gap:6,alignItems:"center",border:"1px solid rgba(255,255,255,0.06)" }}>
                  {[0,1,2].map(j => (
            ...
            <div style={{ display:"flex",gap:8 }}>
              <input className="chat-input" value={chatInput} onChange={e => setChatInput(e.target.value)}
                ...
                style={{ flex:1,padding:chatMax?"14px 18px":"10px 14px",fontSize:chatMax?16:14,border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:14,
                  outline:"none",fontFamily:FF,background:"rgba(30,41,59,0.6)",color:"#f1f5f9",transition:"all 0.2s" }} />
              <button onClick={sendChat} disabled={chatLoading || !chatInput.trim()}
                style={{ width:42,height:42,borderRadius:14,border:"none",cursor:chatInput.trim()?"pointer":"default",
                  background:chatInput.trim()?"linear-gradient(135deg,#0071e3,#7c3aed)":"rgba(30,41,59,0.8)",
                  color:chatInput.trim()?"#fff":"#64748b",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",
        ...
        <div style={{ ...S.inner,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12 }}>
          ...
          <div style={{ display:"flex",gap:16,flexWrap:"wrap" }}>
            {[["home","Overview"],["topics","Topics"],["scenarios","Scenarios"],["blog","Blog"],["quiz","Quiz"],["community","Community"],["weekly","Weekly"]].map(([p,l]) => (
              <button key={p} onClick={() => goPage(p)} style={{ background:"none",border:"none",color:"#94a3b8",fontSize:12,cursor:"pointer",fontFamily:FF }}>{l}</button>
            ))}
          </div>
        </div>
      </footer>
...
const S = {
  ...
  navLink:{ background:"none",border:"none",cursor:"pointer",fontSize:12.5,padding:"6px 9px",borderRadius:6,fontFamily:FF,transition:"color .2s",whiteSpace:"nowrap",flexShrink:0 },
  ...
  tcSub:{ fontSize:12,color:"#94a3b8",marginBottom:6,lineHeight:1.4 },
